import React from 'react';
import { Nav } from 'react-bootstrap';
import { Link, useLocation } from 'react-router-dom';
import { FaHome, FaUserInjured, FaCalendarAlt, FaFileAlt, FaComments, FaUser, FaPrescriptionBottleAlt, FaSignOutAlt } from 'react-icons/fa';
import { useAuth } from '../../contexts/AuthContext';

const DoctorSidebar = () => {
  const location = useLocation();
  const { logout } = useAuth();

  const isActive = (path) => {
    return location.pathname === `/doctor${path}`;
  };

  return (
    <div className="sidebar">
      <div className="p-3">
        <h4 className="text-white mb-4">Doctor Panel</h4>
        <Nav className="flex-column">
          <Nav.Link 
            as={Link} 
            to="/doctor" 
            className={isActive('') ? 'active' : ''}
          >
            <FaHome className="me-2" />
            Dashboard
          </Nav.Link>
          
          <Nav.Link 
            as={Link} 
            to="/doctor/patients" 
            className={isActive('/patients') ? 'active' : ''}
          >
            <FaUserInjured className="me-2" />
            My Patients
          </Nav.Link>
          
          <Nav.Link 
            as={Link} 
            to="/doctor/appointments" 
            className={isActive('/appointments') ? 'active' : ''}
          >
            <FaCalendarAlt className="me-2" />
            Appointments
          </Nav.Link>
          
          <Nav.Link 
            as={Link} 
            to="/doctor/medical-records" 
            className={isActive('/medical-records') ? 'active' : ''}
          >
            <FaFileAlt className="me-2" />
            Medical Records
          </Nav.Link>
          
          <Nav.Link 
            as={Link} 
            to="/doctor/messages" 
            className={isActive('/messages') ? 'active' : ''}
          >
            <FaComments className="me-2" />
            Messages
          </Nav.Link>
          
          <Nav.Link 
            as={Link} 
            to="/doctor/prescriptions" 
            className={isActive('/prescriptions') ? 'active' : ''}
          >
            <FaPrescriptionBottleAlt className="me-2" />
            Prescriptions
          </Nav.Link>
          
          <Nav.Link 
            as={Link} 
            to="/doctor/profile" 
            className={isActive('/profile') ? 'active' : ''}
          >
            <FaUser className="me-2" />
            My Profile
          </Nav.Link>
          
          <Nav.Link 
            onClick={logout} 
            style={{ cursor: 'pointer' }}
          >
            <FaSignOutAlt className="me-2" />
            Logout
          </Nav.Link>
        </Nav>
      </div>
    </div>
  );
};

export default DoctorSidebar; 